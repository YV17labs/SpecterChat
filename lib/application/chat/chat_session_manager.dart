...
class ChatSessionManager {
  ...
  ChatSession getOrCreate(String conversationId) {
    ...
    // Make room first: the new session must never be an eviction
    // candidate on its own creation.
    _evictIfNeeded(reserve: 1);
    ...
    _log.fine(
      'Created session $conversationId '
  ...
  /// deletes a conversation: an in-flight stream is cancelled so nothing
  /// tries to write to rows that no longer exist. No-op for unknown ids.
  Future<void> disposeSession(String conversationId) async {
    final session = _sessions.remove(conversationId);
    if (session == null) return;
    await session.dispose();
    _log.fine(
      'Disposed session $conversationId '
      '(${_sessions.length} active)',
    );
  }
  ...
  Future<void> _hydrate(ChatSession session) async {
    try {
      ...
      final conv = await deps.conversations.getConversation(
        session.conversationId,
      );
      if (conv != null) session.hydrate(conv);
    } catch (e, st) {
  ...
  /// a live listener. [reserve] is the number of slots about to be taken.
  void _evictIfNeeded({int reserve = 0}) {
    if (_sessions.length + reserve <= maxActiveSessions) return;
    ...
    final candidates =
        _sessions.values
            .where(
              (s) =>
                  !s.isGenerating &&
                  (_observerCount[s.conversationId] ?? 0) == 0,
            )
            .toList()
          ..sort((a, b) => a.lastActivity.compareTo(b.lastActivity));

    var over = _sessions.length + reserve - maxActiveSessions;
    for (final s in candidates) {
      if (over <= 0) break;
      _log.fine(
        'LRU evicting ${s.conversationId} '
