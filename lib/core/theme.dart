...
class SpecterStyles extends ThemeExtension<SpecterStyles> {
  ...
  /// Secondary text (labels, subtitles): `onSurface` at 60 %.
  final Color textMuted;

  /// Tertiary text (hints, previews, stats): `onSurface` at 40 %.
  final Color textFaint;

  /// In-between (icons on hover, secondary buttons): `onSurface` at 50 %.
  final Color textSubtle;

  /// 11 px, muted. Timestamps, token stats, tooltips.
  final TextStyle caption;

  /// 12 px, default foreground. Dense list rows, form fields.
  final TextStyle small;

  /// 12 px, muted. Field labels, descriptions.
  final TextStyle smallMuted;

  /// 12 px monospace for JSON payloads and URIs.
  final TextStyle monospace;

  /// 10 px uppercase tracked label for section headings inside cards.
  ...
  SpecterStyles copyWith({
    ...
    return SpecterStyles(
      ...
      smallMuted: smallMuted ?? this.smallMuted,
  ...
  SpecterStyles lerp(ThemeExtension<SpecterStyles>? other, double t) {
    ...
    return SpecterStyles(
      toolResultGroupDecoration:
          BoxDecoration.lerp(
            toolResultGroupDecoration,
            other.toolResultGroupDecoration,
            t,
          ) ??
          toolResultGroupDecoration,
      expandableBlockDecoration:
          BoxDecoration.lerp(
            expandableBlockDecoration,
            other.expandableBlockDecoration,
            t,
          ) ??
          expandableBlockDecoration,
      thinkingBlockDecoration:
      ...
      blockquotePadding:
          ...
          blockquotePadding,
      sidebarBorderSide: BorderSide.lerp(
        sidebarBorderSide,
        other.sidebarBorderSide,
        t,
      ),
      markdownStyleSheet: t < 0.5
          ? markdownStyleSheet
      ...
      smallMuted: TextStyle.lerp(smallMuted, other.smallMuted, t) ?? smallMuted,
      monospace: TextStyle.lerp(monospace, other.monospace, t) ?? monospace,
      sectionLabel:
...
extension SpecterStylesX on BuildContext {
  SpecterStyles get specterStyles => Theme.of(this).extension<SpecterStyles>()!;
}
...
class SpecterTheme {
  ...
  static ThemeData get darkTheme {
    ...
    final baseTheme = ThemeData(
      ...
      scrollbarTheme: ScrollbarThemeData(
        thumbColor: WidgetStateProperty.all(
          _OneDark.border.withValues(alpha: 0.8),
        ),
    ...
    const blockquotePadding = EdgeInsets.symmetric(horizontal: 12, vertical: 8);
    ...
    final markdownStyleSheet = MarkdownStyleSheet.fromTheme(baseTheme).copyWith(
    ...
    final textFaint = colorScheme.onSurface.withValues(alpha: 0.4);
    final textSubtle = colorScheme.onSurface.withValues(alpha: 0.5);
    ...
    final specterStyles = SpecterStyles(
      ...
      caption: TextStyle(fontSize: 11, color: textFaint),
      small: const TextStyle(fontSize: 12, color: _OneDark.text),
      smallMuted: TextStyle(fontSize: 12, color: textMuted),
      monospace: TextStyle(
        fontFamily: 'monospace',
        fontSize: 12,
