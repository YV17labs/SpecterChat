...
/// Persisted configuration of one MCP server. Runtime state (connection,
/// tool list, prompts…) lives in `McpServerState` and is never written to
/// disk — a connection does not survive an app restart.
///
/// Unknown keys in stored JSON (from older versions that persisted runtime
/// fields here) are ignored on load.
@freezed
abstract class McpServerConfig with _$McpServerConfig {
  const factory McpServerConfig({
    ...
    /// Global on/off switch. A disabled server is never connected
    /// automatically and never exposes tools, even if a conversation lists
    /// it in its `enabledMcpServerIds`.
