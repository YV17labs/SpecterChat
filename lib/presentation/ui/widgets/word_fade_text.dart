...
class _WordFadeTextState extends State<WordFadeText>
  ...
  void initState() {
    ...
    if (!widget.isStreaming) {
      _emitTimes.addAll(List.filled(_tokens.length, Duration.zero));
  ...
  void didUpdateWidget(covariant WordFadeText oldWidget) {
    ...
    var canExtend = emitted <= newTokens.length;
    if (canExtend) {
  ...
  bool _hasInFlightFade() {
    ...
    for (var i = _settledIdx; i < _emitTimes.length; i++) {
  ...
  void _onTick(Duration elapsed) {
    ...
    if (!widget.isStreaming) {
      ...
      final t =
          ((pending - _reliefThreshold) / (_maxPending - _reliefThreshold))
              .clamp(0.0, 1.0);
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = _settledIdx; i < emitted; i++) {
      final elapsed = (_now - _emitTimes[i]).inMicroseconds;
      final raw = fadeMicros > 0 ? (elapsed / fadeMicros).clamp(0.0, 1.0) : 1.0;
